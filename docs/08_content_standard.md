...
本规范是 Become Engineer 后续课程正文、公开笔记、练习和项目章节的统一内容基线。新内容从创建时执行；已开放的 49 节正式课程按已验收的任务驱动与知识库助教契约迁移，其余历史内容仍在下一次扩写或重构时逐步迁移。
...
- 基础课程：所有路线共享的事实、任务、失败路径和迁移验收。
- 新手操作包：安装、寻找软件、打开界面和首次成功；优先进入工程基础正文或可折叠补充。
- 深化挑战包：原理、边界、性能、复杂度和工程权衡，不与求职目的绑定。
- 求职训练包：机考、限时模拟、项目证据问答、失败复盘和迁移题，只在求职路线主动推荐。

正式内容必须登记稳定 ID、URL、能力维度、选择规则、公开状态和前置。面试问题必须关联课程或项目中的代码、测试、图、日志或故障证据，不建立脱离实践的孤立背诵区。企业题目只有来源、授权与核查日期明确时才标记企业；否则按能力模型独立改写。

### 求职题库参考素材

- 官方公开 API 或许可证明确的开源仓库可以按最小必要范围自动读取；平台题库、社区频率和用户面经只允许人工浏览、链接登记与独立能力摘要。
- 登录后、付费、验证码、反爬限制、未公开接口、企业内部材料和个人信息不得采集。
- 本地参考信号可以保留题目编号、标题、标签、难度、输入形式和定性频率，但不得保存受限来源的完整题面、答案、题解代码或面经正文。
- 社区频率只能标记为社区信号，不能写成企业官方题库、真实出现概率或确定性招聘结论。
- 原创题从能力目标开始，重新设计场景、约束、输入输出、测试、失败路径和证据；公开前必须完成来源追踪、相似性复核和学习者试做。
- V2 将信号分为方向、重复和高频参考：单一来源只能说明方向，两个来源族表示重复，至少三个来源族且包含 A 级来源才能标记高频参考。
- 高频证据和课程重点必须分开。重点课程还要考虑前置中心性、岗位覆盖、项目证据和当前课程缺口，不按外部记录数量直接排序。
- 使用求职素材时必须通过课程生产 Skill 的脱敏查询脚本；不得直接打开 raw 缓存、受限平台正文或无许可仓库内容来改写题目。
- 每道原创题至少引用 3 条能力信号和 2 个来源族，记录原创场景、失败路径、测试或评分证据；未达到门槛时只登记内容缺口。

## 核心原则

- **任务先行，知识按需进入。** 编程与项目课程优先让学习者先得到可观察结果，再围绕修改、失败和判断补充最少必要知识。
...
已挂载知识库助教的正式课程还必须提供稳定课程标识、5–7 个步骤锚点、每课至少 8 张审核知识卡和 16 条固定问法；知识卡只指向课程正文，未知问题必须明确降级。

正式课程的新建或重构必须使用仓库内 `$become-engineer-course-authoring` Skill。该 Skill 不代替事实核查或人工审阅，但负责把课程正文、助教数据、检索测试和站点接入按同一契约交付。

编程课和项目课优先组织为“任务 -> 结果 -> 最少知识 -> 主动修改 -> 失败排查 -> 迁移任务 -> 验收”。概念关系仍需完整，但不要求全部位于第一次操作之前。
