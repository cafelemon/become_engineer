...
- 规范、模板、Skill 和双协议运行时已经固化；55 节既有课程、Web 起步四课、Web 核心四课及 CS 系统基础六课均按同一生产链完成，正式登记达到 69 节。
- Web 起步 v0.4 采用单一 FastAPI 同源提供静态页面与 JSON 接口；Web 核心 v0.5 加入 TypeScript、OpenAPI 和运行时契约，v0.6 加入 SQLite 关系模型与事务持久化，v0.7 加入 REST 资源、游标分页和重复写入保护，v0.8 完成表单状态、四层校验、可重试提交与服务器重读。认证、跨来源部署与可观测性继续在 CS 前置与 Web 工程化层进入。
