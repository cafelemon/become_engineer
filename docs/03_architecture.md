...
- 任务驱动契约已经覆盖现行 55 节课程，并作为 V1 兼容维护；新建和迁移课程执行 V2 语义内容契约。
...
- 规范、模板、Skill 和双协议运行时已经固化；55 节正式课程完成逐课迁移审计，正文、知识库、导航和 URL 仍未改变。
...
55 节现有课程仍使用 V1；V2 已成为新建与迁移目标。运行时和校验器同时支持两者，直到正式课程全部迁移完成。
