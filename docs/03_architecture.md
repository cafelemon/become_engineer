...
`site-src/data/curriculum/v2.json` 登记模块 ID、层级、角色、前置、解锁、四类用户规则、内容状态、衔接状态和 55 节课程 URL。`scripts/validate_curriculum_v2.cjs` 校验登记与真实 Markdown、一级标题和权威地图链接一致；个性化 V1 暂不消费该文件。
...
- 任务驱动与助教样板已经完成现行 55 节课程覆盖；该契约在内容架构样板通过前继续兼容维护，但不再作为所有未来课型的目标结构。
- `.agents/skills/become-engineer-course-authoring/` 是课程生产流程的仓库内唯一源，个人 Codex Skills 只保留其经校验的可用副本。

## 课程内容架构修订边界

[课程内容架构修订总计划](10_course_content_revision_plan.md)定义目标学习体验和分批迁移，不直接成为当前运行时接口或正式生产规范。
...
以上为现行 V1 契约。目标语义上下文、双协议兼容和新的静态校验以内容样板评审结果为准，不在总计划落地阶段提前实现。
