...
- 规范、模板、Skill 和双协议运行时已经固化；55 节既有课程与后续课程组均按同一生产链建设，数学数据实验进入 2/6 后正式登记达到 97 节。
...
- 数学数据实验采用“可复现实验与评估系统”项目线：v0.1 用 Python 标准库冻结向量长度、矩阵形状、点积、矩阵向量积和距离契约；v0.2 增加训练集拟合的标准化器、总体标准差口径和零方差拒绝，后续加入数据质量、划分、指标与实验产物。
