...
- 规范、模板、Skill 和双协议运行时已经固化；55 节既有课程、Web 起步四课与 Web 核心前三课均按同一生产链完成，正式登记达到 62 节。
- Web 起步 v0.4 采用单一 FastAPI 同源提供静态页面与 JSON 接口；Web 核心 v0.5 加入 TypeScript、OpenAPI 和运行时契约，v0.6 加入 SQLite 关系模型与事务持久化，v0.7 加入 REST 资源、游标分页和重复写入保护。表单状态、认证、跨来源部署与可观测性继续按核心和工程化层逐步进入。
