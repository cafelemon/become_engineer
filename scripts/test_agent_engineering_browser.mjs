...
const lessonRoot = `${base}/learning-paths/llm-agent/agent-engineering`;
const slugs = [
  "01-sqlite-run-state-event-log-atomic-checkpoint",
  "02-memory-provenance-consent-ttl-context-budget",
  "03-lease-idempotent-step-crash-resume",
  "04-trajectory-outcome-recovery-memory-evaluation",
  "05-trace-span-correlation-metrics-redaction",
  "06-injection-isolation-security-release-rollback",
];
const modes = [
...
for (const mode of modes) {
  ...
  for (const slug of slugs) {
    ...
    const audit = await page.evaluate(() => ({
      ...
      project: document.body.innerText.includes("可评估的智能学习助手"),
      command: document.body.innerText.includes("python3"),
      source: document.body.innerText.includes("来源与版本"),
    }));
...
if (focused !== "打开学习助教") throw new Error(`keyboard focus did not return: ${focused}`);
await keyboard.close();
await browser.close();

console.log(JSON.stringify({valid:true,page_checks:pageChecks,modes:modes.map((mode)=>mode.name),keyboard_focus:focused},null,2));
