...
const lessonRoot = `${base}/learning-paths/ai-foundation/math-data`;
const slugs = [
  "01-vectors-matrices-shapes-numeric-contracts",
  "02-dot-distance-scaling-standardization",
  "03-table-schema-missing-duplicates-data-quality",
  "04-randomness-sampling-train-validation-leakage",
  "05-baseline-confusion-matrix-threshold-metrics",
  "06-experiment-config-data-fingerprint-artifact-reproduction",
];
const modes = [
...
for (const mode of modes) {
  ...
  for (const slug of slugs) {
    ...
    const audit = await page.evaluate(() => ({
      ...
      project: document.body.innerText.includes("可复现实验与评估系统"),
      command: document.body.innerText.includes(".venv/bin/python"),
    }));
    if (!audit.title || audit.overflow || !audit.profiles || !audit.project || !audit.command) {
