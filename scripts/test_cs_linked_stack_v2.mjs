...
const micro = join(root, "site-src/examples/algorithm-foundation/linked_stack_trace.py");
const temp = mkdtempSync(join(tmpdir(), "be-cs-stack-"));
...
try {
  ...
  expectSuccess(microRun, "链式栈微型例子运行失败");
  assert.equal(microRun.stdout, [
    "push 7  stack=[7] size=1",
    "push 3  stack=[3, 7] size=2",
    "pop -> 3 stack=[7] size=1",
    "push 9  stack=[9, 7] size=2",
    "peek -> 9 size=2",
    "",
  ].join("\n"));
  ...
  console.log("栈、LIFO 接口与空栈边界验证通过");
} finally {
