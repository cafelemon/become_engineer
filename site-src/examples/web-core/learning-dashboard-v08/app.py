...
class StudySessionInput(BaseModel):
    ...
    hours: Annotated[float, Field(gt=0, le=24, multiple_of=0.25)]
    note: Annotated[str, Field(min_length=2, max_length=200)]
    ...
    def normalize_note(cls, value: str) -> str:
        ...
        if len(normalized) < 2:
            raise ValueError("备注去掉空格后至少需要 2 个字符")
        return normalized
...
def create_app(database_path: Path | None = None) -> FastAPI:
    ...
    api = FastAPI(
        title="学习进度报告器 API",
        version="0.8.0",
        description="Web Core v0.8：表单校验、可重试提交和服务器状态同步。",
    )
