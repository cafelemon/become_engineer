...
class LearningDashboardRestTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def create_session(
        self,
        key: str = "lesson-v07-request-001",
        *,
        hours: float = 1.0,
        note: str = "REST 与幂等练习",
    ):
    ...
    def test_health_reports_schema_two(self) -> None:
    ...
    def test_created_resource_persists_in_new_database_instance(self) -> None:
        response = self.create_session("lesson-v07-restart")
        session_id = response.json()["session"]["session_id"]
        reopened = LearningDatabase(self.database_path)
        self.assertEqual(reopened.get_session(session_id).note, "REST 与幂等练习")
    ...
    def test_static_dashboard_is_served(self) -> None:
        ...
        self.assertIn("学习进度报告器 Web v0.7", page.text)
