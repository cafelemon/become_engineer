...
import { render, renderSave, type DashboardRefs } from "./render.js";
...
const profileButtons = Array.from(
  document.querySelectorAll<HTMLButtonElement>("[data-profile-id]")
);
const saveButton = requiredElement<HTMLButtonElement>("[data-save-one-hour]");
...
async function loadProfile(profileId: string): Promise<void> {
  ...
  if (result.kind === "success") activeLearnerId = result.record.learnerId;
...
saveButton.addEventListener("click", async () => {
  ...
  if (result.kind === "saved") await loadProfile(activeLearnerId);
