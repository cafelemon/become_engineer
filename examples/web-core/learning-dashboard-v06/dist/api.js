import { isLearningSummary, isStudySessionCreated, summaryToRecord } from "./contracts.js";
const API_ROOT = "/api";
export function loadingState(profileId) {
    return { kind: "loading", message: `正在从 SQLite 读取 ${profileId}……` };
}
export function endpointFor(profileId) {
    if (profileId === "unavailable")
        return `${API_ROOT}/demo-unavailable`;
...
export async function saveStudySession(learnerId, hours, note, fetchImpl = fetch) {
    let response;
    try {
