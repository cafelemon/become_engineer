...
import { clearSessionDraft, createSubmissionIntent, LatestRequestGate, validateSessionDraft } from "./form.js";
import { renderForm, renderSessions, renderSummary, showRefreshOnly } from "./render.js";
...
const refs = {
    ...
    form: requiredElement("[data-session-form]"),
    submitButton: requiredElement("[data-submit-session]"),
    refreshButton: requiredElement("[data-refresh-dashboard]"),
...
const hoursInput = requiredElement("[data-input-hours]");
const noteInput = requiredElement("[data-input-note]");
...
async function refreshProfile(profileId) {
    ...
    if (!refreshGate.isCurrent(token))
        return false;
    ...
    if (summary.kind === "success")
        activeLearnerId = summary.record.learnerId;
    for (const button of profileButtons)
...
function nativeErrors() {
    const errors = {};
    if (!hoursInput.validity.valid)
        errors.hours = hoursInput.validationMessage;
    if (!noteInput.validity.valid)
        errors.note = noteInput.validationMessage;
    return errors;
}
for (const button of profileButtons) {
    button.addEventListener("click", async () => {
        const profileId = button.dataset.profileId;
        if (!profileId || submitting || waitingForRefresh)
            return;
...
refs.form.addEventListener("submit", async (event) => {
    ...
    if (!refreshed) {
        showRefreshOnly(`记录 #${result.result.session.session_id} 已保存，但页面没有重新同步。请只点“重新同步”，不要再次提交。`, refs);
        return;
    }
    refs.form.reset();
