...
function fieldMessage(field: SessionField, errorType: string, fallback: string): string {
  if (field === "hours") {
    if (errorType.includes("multiple_of")) return "小时数需要按 0.25 递增。";
    return "服务器要求小时数大于 0、不超过 24。";
  }
  if (errorType.includes("too_long")) return "服务器要求备注不超过 200 个字符。";
  if (errorType.includes("too_short")) return "服务器要求备注至少 2 个字符。";
  if (errorType.includes("value_error")) return "备注不能只包含空格。";
  return fallback;
}

export function serverFieldErrors(payload: unknown): FieldErrors {
  if (!isRecordValue(payload) || !Array.isArray(payload.detail)) return {};
  ...
  for (const item of payload.detail) {
    ...
    const fieldValue: unknown = item.loc.at(-1);
    if (fieldValue !== "hours" && fieldValue !== "note") continue;
    const field: SessionField = fieldValue;
