...
本单元先建立 Docker 的最小认知：它不是必须一开始就精通的工具，但后面做 Web、AI、LLM 和部署时会经常遇到。

阶段 0 只要求你知道 Docker 解决什么问题，能读懂最常见的几个词：镜像、容器、端口、挂载和环境变量。复杂的 Dockerfile、docker compose 和生产部署放到后续项目中学习。

## 前置知识

开始前应完成：

- [单元 1 学习方法](01-learning-method.md)
- [单元 2 文件系统](02-filesystem.md)
- [单元 3 终端与 Shell](03-terminal-shell.md)
- [单元 4 编辑器](04-editor.md)
- [单元 5 Markdown](05-markdown.md)
- [单元 6 Git](06-git.md)
- [单元 7 开发环境](07-development-environment.md)

你需要能打开终端，能执行命令并记录输出。没有安装 Docker 也可以完成本单元的大部分练习。
...
本单元不要求写 Dockerfile，不要求配置 docker compose，也不要求部署真实项目。
...
如果你能把命令拆开解释，就已经完成阶段 0 的 Docker 最小目标。
...
| 一开始就学复杂部署 | 概念混在一起，难以排查 | 阶段 0 只掌握镜像、容器、端口、挂载和环境变量 |
...
进入 [单元 9 验证习惯](09-validation-habit.md)。下一单元会把前面学到的命令、文件、环境和 Docker 认知串起来，学习如何运行、观察输出、复现错误、记录命令和结果。
