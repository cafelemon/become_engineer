...
本单元解决一个最常见的小白困惑：同样一段代码，为什么有的人能运行，有的人运行不了。
...
本单元不要求你安装任何新工具，也不要求你解决所有环境问题。目标是先会看、会问、会记录。
...
本单元只要求你知道 PATH 的作用，不要求你手动修改 PATH。随意修改 PATH 可能导致原本能用的命令失效。
...
本单元只建立认知，不要求你马上创建虚拟环境。真正的 Python 虚拟环境操作会在 Python 起步阶段学习。
...
| 盲目修改 PATH | 原本能用的命令也失效 | 本阶段只查看 PATH，不修改 PATH |
| 把依赖装到全局环境 | 后续项目互相影响 | 先理解虚拟环境，后续在 Python 阶段练习 |
...
进入 [单元 8 Docker最小认知](08-docker-basics.md)。下一单元会学习 Docker 解决什么问题，以及镜像、容器、端口、挂载和环境变量的基本含义。
