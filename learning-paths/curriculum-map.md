...
[直接看 114 节明细](#opened-lessons){ .md-button }
...
当前正式课程共 **129 节**：工程基础 10 节、Python 起步 7 节、CS 起步 4 节、首个可验证项目 1 节、Python 核心与工程化 7 节、C++ 起步 2 节、C++ 核心 3 节、算法基础与核心 22 节、算法深化 10 节、算法求职加练 3 节、CS 系统基础 6 节、系统工程 6 节、AI 数学数据 6 节、机器学习 8 节、深度学习 8 节、模型使用与结构化输出 6 节、检索/RAG/评估 6 节、Web 起步 4 节、Web 核心 4 节、Web 工程化 6 节。
...
<div class="be-architecture-flow">
  <section class="be-flow-foundation" aria-labelledby="flow-foundation-title">
    ...
    <div class="be-flow-sequence">
      ...
      <span aria-hidden="true">→</span>
      <a href="#module-cs-start" data-status="open"><strong>CS 起步</strong><small>已开放 · 可顺序进入</small></a>
      <span aria-hidden="true">→</span>
      <a href="#module-direction-choice" data-status="open"><strong>首个可验证项目</strong><small>已开放 · 用两周实验选择方向</small></a>
    </div>
  </section>
  ...
  <div class="be-flow-branches">
    ...
    <section>
      ...
      <a href="#module-systems-engineering" data-status="open">并发、网络与性能 · 6 / 6</a>
    </section>
    <section>
      <h3>算法</h3>
      <a href="#module-algorithm-foundation" data-status="open">共同算法基础</a>
      <a href="#module-algorithm-core" data-status="open">算法核心</a>
      <a href="#module-algorithm-deepening" data-status="open">算法深化 · 已开放 10 / 10</a>
      <a href="#module-career-algorithm" data-status="open">求职：机考与复盘 · 3 / 3</a>
    </section>
    <section>
      <h3>AI 模型</h3>
      <a href="#module-ai-math-data" data-status="open">数学、数据与实验 · 6 / 6</a>
      <a href="#module-machine-learning" data-status="open">机器学习 · 已开放 8 / 8</a>
      <a href="#module-deep-learning" data-status="open">深度学习 · 已开放 8 / 8</a>
...
| <span id="module-cs-start">CS 起步</span> | 起步 | 共同基座 | 能描述表示、复杂度与边界 | Python 起步 → 算法基础、方向选择 | 四类共用；已有基础者可用每课检查跳过已掌握部分 | 已开放 | 可顺序进入 |
| <span id="module-direction-choice">首个可验证项目与方向选择</span> | 基础 | 共同基座 | 第一份代码、测试、说明和复盘证据 | CS 起步 → 各方向 | 四类都完成；求职画像追加项目表达 | 已开放 · 1/1 | 可顺序进入 |
...
| <span id="module-systems-engineering">系统编程、并发、网络与性能</span> | 工程化 | 能力深化 | 可诊断的系统程序与性能证据 | C++ 核心 + CS 系统核心 → 设备／高级 Agent | 系统和设备路线重点；求职叠加故障题 | 已开放 · 6/6 | 方向可选 |
...
| <span id="module-deep-learning">深度学习</span> | 核心 | 方向主干 | 神经网络训练、调试和评估 | 机器学习 → RL／视觉／NLP／多模态 | AI 模型路线进入；LLM 应用不强制 | 已开放 · 8/8 | 可顺序进入 |
...
以下 **136 节课程各出现一次**。既有 55 节已完成 V2 迁移；Agent 工程进入 1/6 建设。
...
1. [浏览器、HTML 与第一张学习卡片](web-fullstack/web-start/01-browser-html-learning-card.md) · 已开放
2. [CSS 布局、视觉层级与手机适配](web-fullstack/web-start/02-css-layout-responsive-dashboard.md) · 已开放
...
1. [从源文件到可执行程序：编译、类型与输入输出](programming-languages/cpp-core/01-build-types-io.md) · 已开放
2. [函数、声明与程序组织](programming-languages/cpp-core/02-functions-declarations-program-organization.md) · 已开放

### C++ 核心 · 3 节

1. [头文件、源文件与最小 CMake 工程](programming-languages/cpp-core/03-headers-sources-cmake.md) · 已开放
2. [C++ STL 容器、迭代器与基础算法](programming-languages/cpp-core/04-stl-containers-iterators-algorithms.md) · 已开放
3. [C++ 对象、引用、指针、生命周期与 RAII](programming-languages/cpp-core/05-objects-references-pointers-lifetime-raii.md) · 已开放

### CS 起步 · 4 节

1. [序列接口、数组表示与安全边界](cs-core/01-sequence-interface-array-representation-boundaries.md) · 已开放
...
1. [首个可验证项目：从代码证据到方向试学](direction-choice/01-first-verifiable-project-direction-trials.md) · 已开放
...
1. [文件描述符、部分 I/O 与所有权](systems-engineering/01-file-descriptors-partial-io-ownership.md) · 已开放
2. [信号、进程监督与优雅停止](systems-engineering/02-signals-process-supervision-graceful-shutdown.md) · 已开放
3. [条件变量、有界队列与关闭协议](systems-engineering/03-condition-variables-bounded-queue-shutdown.md) · 已开放
4. [非阻塞网络、事件循环与背压](systems-engineering/04-nonblocking-network-event-loop-backpressure.md) · 已开放
5. [延迟分布、采样分析与性能预算](systems-engineering/05-latency-distribution-sampling-performance-budget.md) · 已开放
6. [故障注入、资源泄漏与恢复验收](systems-engineering/06-fault-injection-resource-leaks-recovery-acceptance.md) · 已开放

### 共同算法与数据结构基础 · 16 节

1. [动态数组容量、扩容成本与摊还分析](cs-core/05-dynamic-array-capacity-amortized-cost.md) · 已开放
2. [单链表、节点链接与所有权](cs-core/06-singly-linked-list-nodes-ownership.md) · 已开放
3. [栈、LIFO 接口与空栈边界](cs-core/07-stack-lifo-interface-underflow.md) · 已开放
4. [队列、FIFO 与首尾不变量](cs-core/08-queue-fifo-head-tail-invariants.md) · 已开放
5. [哈希函数、键相等与冲突](cs-core/09-hash-function-key-equality-collisions.md) · 已开放
6. [分离链接、负载因子与扩容](cs-core/10-separate-chaining-load-factor-rehash.md) · 已开放
7. [集合去重、频次映射与稳定输出](cs-core/11-set-frequency-map-deterministic-output.md) · 已开放
8. [有序查找、半开区间与左右边界](cs-core/12-ordered-search-half-open-boundaries.md) · 已开放
9. [插入排序、选择排序与稳定性](cs-core/13-insertion-selection-sort-stability.md) · 已开放
10. [自底向上归并排序与稳定复杂度](cs-core/14-bottom-up-merge-sort-stable-complexity.md) · 已开放
11. [二叉树形状、链接所有权与槽位表示](cs-core/15-binary-tree-shape-linked-ownership.md) · 已开放
12. [递归深度优先遍历、基线条件与调用深度](cs-core/16-recursive-dfs-traversal-call-frames.md) · 已开放
13. [迭代 DFS、BFS 与层级前沿](cs-core/17-iterative-dfs-bfs-frontier-levels.md) · 已开放
14. [简单无向图、邻接表示与输入边界](cs-core/18-undirected-graph-adjacency-representations.md) · 已开放
15. [BFS、无权距离与最短路径](cs-core/19-bfs-unweighted-distances-shortest-paths.md) · 已开放
16. [DFS、连通分量与无向环检测](cs-core/20-dfs-connected-components-undirected-cycles.md) · 已开放

### 算法核心 · 6 节

1. [二叉最小堆、隐式树与堆不变量](cs-core/21-binary-min-heap-implicit-tree-invariant.md) · 已开放
2. [稳定优先队列、同优先级顺序与下溢](cs-core/22-stable-priority-queue-tie-order-underflow.md) · 已开放
3. [带权图松弛、Dijkstra 与过期队列项](cs-core/23-weighted-relaxation-dijkstra-stale-entries.md) · 已开放
4. [并查集、按大小合并与路径压缩](cs-core/24-disjoint-set-union-path-compression.md) · 已开放
5. [Kruskal、环检测与最小生成森林](cs-core/25-kruskal-minimum-spanning-forest.md) · 已开放
6. [Lazy Prim、割边前沿与过期边](cs-core/26-lazy-prim-cut-frontier-stale-edges.md) · 已开放

### 算法深化 · 已开放 10 / 10

1. [有序双指针、候选消除与不变量](algorithm-deepening/01-sorted-two-pointers-candidate-elimination-invariant.md) · 已开放
2. [滑动窗口、频次状态与最短覆盖](algorithm-deepening/02-sliding-window-frequency-minimum-cover.md) · 已开放
3. [前缀和、差分与区间批处理](algorithm-deepening/03-prefix-sum-difference-range-batching.md) · 已开放
4. [单调栈、单调队列与支配关系](algorithm-deepening/04-monotonic-stack-queue-dominance.md) · 已开放
5. [回溯、选择树与剪枝](algorithm-deepening/05-backtracking-choice-tree-pruning.md) · 已开放
6. [贪心选择、交换论证与反例](algorithm-deepening/06-greedy-choice-exchange-counterexample.md) · 已开放
7. [线性动态规划、状态与转移](algorithm-deepening/07-linear-dynamic-programming-state-transition.md) · 已开放
8. [背包、区间动态规划与空间压缩](algorithm-deepening/08-knapsack-interval-dp-space-compression.md) · 已开放
9. [高阶图：拓扑、强连通与多源路径](algorithm-deepening/09-advanced-graph-topology-scc-multisource.md) · 已开放
10. [Trie、KMP 与字符串匹配](algorithm-deepening/10-trie-kmp-string-matching.md) · 已开放

### 算法求职加练 · 已开放 3 / 3

1. [固定输入输出与本机判题契约](career-algorithm/01-fixed-io-local-judge-contract.md) · 已开放
...
1. [张量、形状、数据类型与设备契约](ai-foundation/deep-learning/01-tensor-shape-dtype-device-contract.md) · 已开放
2. [线性层、激活函数、参数与前向图](ai-foundation/deep-learning/02-linear-layer-activation-parameters-forward-graph.md) · 已开放
3. [交叉熵、自动微分、反向传播与梯度核查](ai-foundation/deep-learning/03-cross-entropy-autograd-backprop-gradient-check.md) · 已开放
4. [Mini-batch、SGD、学习率与训练循环](ai-foundation/deep-learning/04-mini-batch-sgd-learning-rate-training-loop.md) · 已开放
5. [验证曲线、过拟合、权重衰减与 Dropout](ai-foundation/deep-learning/05-validation-curves-overfitting-weight-decay-dropout.md) · 已开放
6. [初始化、激活分布、梯度诊断与裁剪](ai-foundation/deep-learning/06-initialization-activation-gradient-diagnostics-clipping.md) · 已开放
7. [检查点、随机状态与精确恢复训练](ai-foundation/deep-learning/07-checkpoint-rng-resume-exact-training.md) · 已开放
8. [Eval、推理 Schema、Manifest 与离线交付](ai-foundation/deep-learning/08-eval-inference-schema-manifest-delivery.md) · 已开放

### 模型使用与结构化输出 · 已开放 6 / 6

1. [模型边界、消息与离线适配器](llm-agent/model-use/01-model-boundary-messages-offline-adapter.md) · 已开放
2. [Prompt 角色、版本、参数与上下文预算](llm-agent/model-use/02-prompt-roles-version-parameters-context-budget.md) · 已开放
3. [JSON、严格 Schema、语义校验与缺失信息](llm-agent/model-use/03-json-schema-semantic-validation-missing-information.md) · 已开放
4. [错误分类、Deadline、退避与有界重试](llm-agent/model-use/04-error-taxonomy-deadline-backoff-bounded-retry.md) · 已开放
5. [流事件、Delta 顺序、终止原因与取消](llm-agent/model-use/05-stream-delta-order-finish-cancel.md) · 已开放
6. [Provider 配置、秘密、脱敏与可选真实验收](llm-agent/model-use/06-provider-config-secrets-redaction-delivery.md) · 已开放

### 检索、RAG 与评估 · 已开放 6 / 6

1. [语料文档契约、来源身份与可复现索引](llm-agent/rag-eval/01-corpus-document-source-reproducible-index.md) · 已开放
2. [倒排索引、BM25、Top-k 与稳定排序](llm-agent/rag-eval/02-inverted-index-bm25-top-k-stable-ranking.md) · 已开放
3. [分块、重叠、来源坐标与精确引用](llm-agent/rag-eval/03-chunk-overlap-source-coordinates-citation.md) · 已开放
4. [Embedding 适配器、余弦相似度与 RRF 混合检索](llm-agent/rag-eval/04-embedding-cosine-rrf-hybrid-retrieval.md) · 已开放
5. [有证据回答、Claim-Citation 门禁与缺证据拒答](llm-agent/rag-eval/05-grounded-claim-citation-abstention.md) · 已开放
6. [固定评估集、检索指标、引用质量与回归门禁](llm-agent/rag-eval/06-fixed-eval-retrieval-citation-regression-gates.md) · 已开放

### Tool Calling 与有界工作流 · 已开放 6 / 6

1. [工具定义、注册表、候选调用与严格参数](llm-agent/tool-calling-workflow/01-tool-definition-registry-candidate-call-strict-arguments.md) · 已开放
2. [业务校验、主体授权与只读工具执行](llm-agent/tool-calling-workflow/02-business-validation-authorization-read-only-execution.md) · 已开放
3. [多工具调用、call_id 关联与部分失败隔离](llm-agent/tool-calling-workflow/03-multiple-calls-call-id-correlation-partial-failure.md) · 已开放
4. [受控副作用、人工确认与幂等保护](llm-agent/tool-calling-workflow/04-controlled-side-effects-confirmation-idempotency.md) · 已开放
...
- **共同基座**：CS 起步四课与首个可验证项目均已开放；四类学习者先形成代码、测试、说明和复盘证据，再用短期实验选择方向。
- **应用工程**：Web 起步、Web 核心、CS 系统基础与 Web 工程化均已完成；RAG 与 Agent 的 Web 工程化前置已满足。
...
- **Agent 工程**：Tool Calling 与有界工作流 6/6 已开放；状态／记忆／上下文／恢复／评估／可观测性／安全进入 1/6 建设。
...
3. 共同基座至 Tool Calling 与有界工作流均已开放；Agent 工程进入 1/6 建设。
