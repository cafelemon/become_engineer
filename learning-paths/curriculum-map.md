...
| Python 核心 | 类型接口、模块边界、迭代器、数据类资源、装饰器上下文、包与 CLI、配置日志，共 7 课 | mypy、虚拟环境、editable/wheel 安装和命令帮助 | 协议、惰性计算、资源边界、类型安全包装与诊断契约 | 项目证据问答已开放；语言专项题待补 | 可安装的双语言学习进度报告器 | 最小工程化主干已开放 |
| CS 必修 | 数据结构、算法、系统、网络、数据库最小核心 | 可视化执行过程和最小实验环境 | 机考算法、系统编程、网络、数据库和系统设计 | 分级算法题、限时模拟、错题与复杂度复盘 | 数据结构实现与系统实验 | 序列、线性结构、哈希集合、查找排序、树遍历、一般图与非负权最短路前 23 课已开放；其余主干建设中 |
...
| 设备系统与边缘智能 | 嵌入式共同基础、MCU/RTOS、Linux/BSP、工业控制和端侧推理 | 仿真环境起步；二、三级出口使用真实设备 | 驱动、实时性、移植裁剪、性能、可靠性和智能控制 | 需求设计、模块测试、故障复盘与板端项目证据 | 设备模块、BSP 或边缘智能系统 | 跨主线专业领域已规划；课程与硬件待建设 |
| 岗位专项 | 复用共同课程，不复制主干 | 只补岗位工具缺口 | AI 应用、后端、C++ 等专项深化 | 岗位题型、项目表达和模拟面试 | 同一项目按岗位生成不同证据视角 | 通用软件工程样板已开放；专项待建设 |

</div>

当前正式课程共 52 节：工程基础 10 节、Python 起步 7 节、C++ 核心 5 节、Python 核心 7 节、CS 核心 23 节。CS 其余主干、Web、AI、LLM/Agent 和设备系统与边缘智能的结构说明不是已完成课程；个性化结果会明确标记“内容尚未建设”，不会把规划伪装成开放内容。
...
6. **设备系统与边缘智能**：共同主干稳定后再建设设备基础和双平台线；前期允许仿真，专业完成必须提供真机证据。
7. **岗位专项**：只有共同能力和真实项目证据稳定后，才扩展 AI 应用、后端、C++ 和嵌入式岗位训练，避免提前堆积题库。
...
<nav class="be-domain-grid" aria-label="课程领域">
  ...
  <a href="../device-edge-systems/"><strong>设备与边缘</strong><span>MCU、Linux/BSP、工业控制与端侧 AI</span></a>
</nav>
...
flowchart TD
    ...
    CSD --> DEVICE["设备系统共同基础"]
    DEVICE --> MCU["MCU / RTOS"]
    DEVICE --> ELINUX["嵌入式 Linux / BSP"]
    DEVICE --> INDUSTRY["工业控制与实时通信"]
    AI0 --> EDGE["边缘智能 / 嵌入式 AI"]
    MCU --> EDGE
    ELINUX --> EDGE
    RL --> CONTROL["智能控制实验"]
    DEVICE --> CONTROL
...
- 设备系统共同基础，以及 MCU/RTOS、嵌入式 Linux/BSP 和工业控制与实时通信。
- 分布式系统与高性能计算。
- 编译原理与程序语言。
- 网络与系统安全。
...
- 强化学习：单独面向控制、决策和序列交互，可在满足设备前置后继续进入智能控制实验。
- 计算机视觉、NLP与信息抽取、多模态与生成模型、时序与推荐。
- 边缘智能：同时要求 AI 基础与 MCU/RTOS 或嵌入式 Linux/BSP，不以强化学习作为通用前置。
...
该领域不是第七条默认必修，而是跨 C++、CS 与 AI 的后置专业方向。MCU/RTOS、嵌入式 Linux/BSP 和工业控制并列；边缘智能在 AI 基础与设备平台能力汇合后解锁。一级允许仿真，二级独立模块开发和三级 BSP／边缘智能高级必须提交真实设备证据。

[查看完整能力树、硬件口径与三级出口](device-edge-systems/README.md){ .md-button }
...
| 设备系统与边缘智能 | 当前只有一份匿名 Linux/BSP 岗位样本用于能力校准 | 本轮只建立架构；素材、硬件和正式课程另行规划 |
