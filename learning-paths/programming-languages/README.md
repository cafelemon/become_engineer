...
2. [核心语言](cpp-core/README.md)：深入 Python 数据模型和现代 C++ 的类型、对象生命周期、STL与泛型；先启动C++构建与类型，再按能力主题配对推进。
...
2. 完成Python起步后先用[从源文件到可执行程序](cpp-core/01-build-types-io.md)启动C++，随后围绕类型与接口、函数与组织、容器与迭代等能力主题配对推进两门语言；不要求机械轮流。
